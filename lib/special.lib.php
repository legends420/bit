...
function send_smtp_email($to_email, $subject, $body, $reply_email=false, $from_email=false, $is_html=false) {
  ...
  $mail->isSMTP();
  $mail->SMTPDebug = $smtp_config[5];
  $mail->SMTPAuth = $smtp_config[2];
  $mail->SMTPSecure = $smtp_config[6];
  $mail->Host = $smtp_config[0];
  $mail->Port = $smtp_config[1];
  $mail->Username = $smtp_config[3];
  $mail->Password = $smtp_config[4];
