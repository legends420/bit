...
class bitsci {
  ...
  public static function explorer_request($api, $address, $confs, $testnet=false) {
	...
	case 1:
	  if ($testnet) {
	    $url = 'https://tbtc.blockr.io/api/v1/address/balance/';
	  } else {
	    $url = 'https://btc.blockr.io/api/v1/address/balance/';
	  }
	  ...
	  if (isset($result['status']) && $result['status'] == 'success') {
	   ...
	   is_numeric($result['data']['balance'])) {
	...
	case 2:
	  if ($testnet) {
	    $url = 'https://chain.so/api/v2/get_address_balance/BTCTEST/';
	  } else {
  ...
  public static function send_btc_request($addr_str, $confirmations) {

    if (defined('USE_TESTNET') && (USE_TESTNET == true)) {

	  $result = self::explorer_request(1, $addr_str, $confirmations, true);
	  if ($result === false) {
	    return self::explorer_request(2, $addr_str, $confirmations, true);
	  } else {
	    return $result;
	  }
	  ...
	  $result = self::explorer_request(0, $addr_str, $confirmations);
      if ($result === false) {
        $result = self::explorer_request(1, $addr_str, $confirmations);
		if ($result === false) {
		  return self::explorer_request(2, $addr_str, $confirmations);
		} else {
		  return $result;
		}
      } else {
