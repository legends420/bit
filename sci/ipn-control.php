<?php
  ...
  */
  ...
  // $total : the total cost of the order in BTC
          ...
          if (confirm_order($order_id, $tran_id, 'Confirmed', $amount_paid, $currency)) {
			...
			foreach ($items_arr as $key => $item_dat) {
			  ...
			  if ($rss_feed === true) {
			    ...
			    // calculate price of item in BTC
			    $price_btc = get_btc_price($item['FilePrice'], $exch_rate);

			    $now = date("D, d M Y H:i:s T");
			    $rss_src = dirname(__FILE__)."/../inc/rss.inc";
			    $item_name = spec_str($item['FileName']);
			    $rss_item = "<item>\n\t<title>".spec_str(RAW_LANG('SOLD'))." $quant x $item_name ".
				spec_str(RAW_LANG('FOR'))." $price_btc BTC</title>\n\t<link>$site_url?page=item".
