	...
	if (!empty($item_data) && ($item_data !== 'N/A')) {
	  ...
	  if ($item_data['FileMethod'] == 'keys') {
	    ...
	    if (!empty($file_data) && ($file_data !== 'N/A')) {	
		  ...
		  echo '<p><b>'.LANG('KEY_EXPIRES').":</b> $remaining</p>";
		  echo '<p><b>'.LANG('FILE_TYPE').':</b> '.$file_data['FileType'].'</p>';
		  echo '<p><b>'.LANG('FILE_SIZE').':</b> '.$file_data['FileStock'].' MB</p>';
	      echo '<p><b>'.LANG('DOWNLOAD').'</b>:';
