...
|| !isset($_SESSION['cart']) || count($_SESSION['cart']) < 1) {
  ...
  if (!empty($_POST['email'])) {
	if (is_injected($_POST['email']) || (strlen($_POST['email'] > 200))) {  
...
<form name="order_form" method="post" action="">
	...
	<div class="row-fluid">
	  ...
	  <div class="span6">
		...
		<input type="text" name="email" id="email" class="input-large" maxlength="200" value="<?php 
		if (!empty($_POST['email'])) { safe_echo($_POST['email']); } 
		elseif (!empty($_SESSION['user_data']['Email'])) { 
		safe_echo($_SESSION['user_data']['Email']); } ?>" required='required' />
		<br clear="both" />
		<label><?php echo LANG('CONFIRM_EMAIL'); ?>:</label>
		<input type="text" name="c_email" id="c_email" class="input-large" maxlength="200" value="<?php 
		if (!empty($_POST['c_email'])) { safe_echo($_POST['c_email']); }
		elseif (!empty($_SESSION['user_data']['Email'])) { 
		safe_echo($_SESSION['user_data']['Email']); } ?>" required='required' />
		<br clear="both" />
