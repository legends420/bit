  ...
  if (!empty($categories) && ($categories != 'N/A')) {
    ...
    while ($category = mysqli_fetch_assoc($categories)) {
      if ($category['Active']) {	
		...
		$category['CatID']."&amp;action=all'>".safe_str($category['Name']).
	    ...
	    if (!empty($sub_cats) && ($sub_cats != 'N/A')) {
		  ...
		  if ($catc_count > 0) {
		    $col_html[$col_index] .= "<li class='sub_li'><img src='./img/list_dot.png'".
		    "alt='' width='14' height='14' /> <a href='index.php?page=cats&amp;id=".
			$category['CatID']."&amp;action=gen'>".LANG('GENERAL')." ($catc_count)</a></li>\n";
		  }
		  $subc_total = 0;
		  ...
		  while ($row = mysqli_fetch_assoc($sub_cats)) {
		    if ($row['Active']) {
			  ...
			  $subc_total += $subc_count;
			}
		  }
		  $total_cats = $subc_total + $catc_count;
		  $col_html[$col_index] = str_replace('(COUNT-CAT'.$category['CatID'].')', "($total_cats)", $col_html[$col_index]);
		  $col_html[$col_index] .= "</ul>";
		}
		$col_html[$col_index] = str_replace('(COUNT-CAT'.$category['CatID'].')', "($catc_count)", $col_html[$col_index]);
    ...
    if (!isset($_GET['action'])) {
      if ($sel_cat['Parent'] <= 0) {
	    ...
	    if (!empty($sub_cats) && ($sub_cats != 'N/A')) {
		  ...
		  $catc_count = count_file_cat($sel_cat['CatID']);
		  if ($catc_count > 0) {
		    echo "<li class='sub_li'><img src='./img/list_dot.png'".
		    "alt='' width='14' height='14' /> <a href='index.php?page=cats&amp;id=".
		    $sel_cat['CatID']."&amp;action=gen'>".LANG('GENERAL')." ($catc_count)</a></li>";
		  }
		  ...
		  while ($row = mysqli_fetch_assoc($sub_cats)) {
		    if ($row['Active']) {
			  ...
			  safe_str($row['Name'])." (".count_file_cat($row['CatID']).")</a></li>\n"; 
			}
		  }
		  echo "</ul>\n";
