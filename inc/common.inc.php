...
if (isset($market_data[$curr_orig])) {
  ...
  $orig_stats = 'data not found';
}

// get the exchange rates for BTC
$exch_rate = $cust_stats[$price_type];
$exch_orig = $orig_stats[$price_type];
...
if ($page === 'cart') {
  ...
  if (!empty($_GET['add'])) {
	...
	if (!empty($item_obj) && $item_obj !== 'N/A') {
      ...
      $_SESSION['cart'][$item_id]['cat'] = $item_arr['FileCat'];
	  ...
	  $_SESSION['cart'][$item_id]['weight'] = 0;
	  if (isset($_GET['qnty'])) {
	  ...
	  if ($item_arr['FileMethod'] === 'ship') {
	    $_SESSION['cart'][$item_id]['weight'] = $item_arr['FileType'];
	  }
	} else {
